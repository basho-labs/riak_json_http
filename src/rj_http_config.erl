...
is_enabled() ->
    case yokozuna:is_enabled(search) of
        true -> app_helper:get_env(riak_json_http, enabled, true);
        _ -> false
    end.

document_name() ->
    app_helper:get_env(riak_json_http, document_name, "document").

%% internal
auth_section() ->
    app_helper:get_env(riak_json_http, auth,[]).
