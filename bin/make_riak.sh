...
Partial Example: ./bin/$SCRIPT_NAME.sh -m partial -r ~/src/basho/riak/ -j ~/src/basho/riak_json_http/
Partial Example Without Deps: ./bin/$SCRIPT_NAME.sh -m partial -r ~/src/basho/riak/ -j ~/src/basho/riak_json_http/ -s
Full Example: ./bin/$SCRIPT_NAME.sh -m full -r ~/src/basho/riak/ -j ~/src/basho/riak_json_http/
...
Usage: $SCRIPT_NAME [OPTION]... [ARG]...
  ...
  -j    riak_json_http source directory
  -s    skip dependencies
"
...
if [ "${mode}" != 'full' ]; then
    ...
    echo "Cleaning ebin.."
    rm ${riak_path}rel/riak/lib/riak_json_http-1/ebin/*
    rm ${riak_path}rel/riak/lib/riak_json-1/ebin/*
    echo "Copying new ebin"
    cp -R ${json_path}ebin/* ${riak_path}rel/riak/lib/riak_json_http-1/ebin/
    ...
    rm -rf ${riak_path}deps/riak_json_http/
    ...
    cp -R ${json_path} ${riak_path}deps/riak_json_http
